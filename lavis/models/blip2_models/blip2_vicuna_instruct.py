...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def __init__(
        ...
        self.llm_model = LlamaForCausalLM4Ablation.from_pretrained(
    ...
    def forward(self, samples):
        ...
        if samples.get('prompt') is not None or samples.get('prompt') == 'None':
        ...
        with self.maybe_autocast():
            outputs = self.llm_model(
                ...
                return_dict=True,
                labels=targets,
                init_tokens=llm_tokens['input_ids'],
    ...
    def generate(
        ...
        outputs[outputs == -1] = self.llm_tokenizer.pad_token_id
...
class LlamaForCausalLM4Ablation(LlamaForCausalLM):
    def forward(
        self,
        ...
        if labels is not None:
            ...
            word_acc = sum(shift_logits[shift_labels != pad_token_id].max(-1)[1] 
                           == shift_labels[shift_labels != pad_token_id]) / (shift_labels[shift_labels != pad_token_id].view(-1).shape[0]+ 0.001)
            word_acc = word_acc.mean()
            pred_tokens = shift_logits.max(-1)[1]
            if random.random() > 0.95:
                for bid in range(2):
                    logger.info(f"gt: {tokenizer.decode(init_tokens[bid][init_tokens[bid] != pad_token_id])}\npred: {tokenizer.decode(pred_tokens[bid][shift_labels[bid] != pad_token_id])}")
