...
def load_pretrained(
    ...
    if len(pretrained_model) == 0:
        if cfg is None:
            logger.info(f"loading from default config {model.default_cfg}.")
...
def load_pretrained_imagenet(
    ...
    **kwargs,
    ...
    logger.info(f"Loading vit_base_patch16_224 checkpoints.")
...
def load_pretrained_kinetics(
    ...
    **kwargs,
    ...
    try:
        ...
        logger.info("Succeeded in loading Kinetics pre-trained weights.")
    except:
        logger.error("Error in loading Kinetics pre-trained weights.")
