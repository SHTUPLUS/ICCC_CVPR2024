...
class RunnerIter(RunnerBase):
    ...
    def __init__(self, cfg, task, model, datasets, job_id, result_dir=None, output_dir=None):
        super().__init__(cfg, task, model, datasets, job_id, result_dir, output_dir)
        ...
        self.iters_per_inner_epoch = int(
            self.config.run_cfg.get("iters_per_inner_epoch", 1000)
        )
        assert (
    ...
    def train(self):
        ...
        for start_iters in range(
            ...
            if not self.evaluate_only:
                ...
                self._save_checkpoint(start_iters, is_best=False, with_grad=False)

            # evaluation phase
            if len(self.valid_splits) > 0 and ((start_iters+1) % self.config.run_cfg.get('save_epoch', 1)) == 0:
                for split_name in self.valid_splits:
                    ...
                    if val_log is not None:
                        if is_main_process():
                            is_best = False
                            if val_log.get('agg_metrics') is not None:
                                agg_metrics = val_log["agg_metrics"]
                                if agg_metrics > best_agg_metric and split_name == "val":
                                    best_epoch, best_agg_metric = start_iters, agg_metrics
                                val_log.update({"best_epoch": best_epoch})
                            
                            self._save_checkpoint(start_iters, is_best=is_best, with_grad=False)
